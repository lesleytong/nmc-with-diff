...
import edu.ustb.sei.mde.compare.match.eobject.EObjectIndex.Side;
import edu.ustb.sei.mde.compare.match.eobject.internal.ByTypeIndex;
import edu.ustb.sei.mde.compare.match.eobject.internal.MatchAheadOfTime;
...
public class ProximityEObjectMatcher implements IEObjectMatcher, ScopeQuery {
	...
	// lyt
	public void createMatches(Comparison comparisonADD, Iterator<EObject> leftEObjects,
			Iterator<EObject> rightEObjects, Iterator<EObject> originEObjects,
		...
		matchIndexedObjects(comparisonADD, distanceMap);
...
//		restructureMatchModel(comparisonADD);
	...
	// lyt
	private Iterable<EObject> matchList(Comparison comparisonADD, Iterable<EObject> todoList,
		...
		while (todo.hasNext()) {
			...
			EObject container = next.eContainer();
			while (container != null && isInScope(container)) {
				if (comparisonADD.getMatch(container) == null) {
					requiredContainers.add(0, container);
				}
				container = container.eContainer();
			}
	...
	// lyt
	private boolean tryToMatch(Comparison comparisonADD, EObject a, boolean createUnmatches,
			MultiKeyMap<EObject, Double> distanceMap) {
		...
		// lyt
		Map<Side, EObject> closests = index.findClosests(comparisonADD, a, aSide, distanceMap);
