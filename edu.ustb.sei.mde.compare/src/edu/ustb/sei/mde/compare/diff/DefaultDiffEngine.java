...
import static edu.ustb.sei.mde.compare.utils.ReferenceUtil.safeEGet;
...
public class DefaultDiffEngine implements IDiffEngine {
	 ...
	 * {@inheritDoc}
	 * 
	 * @see edu.ustb.sei.mde.compare.diff.IDiffEngine#diff(edu.ustb.sei.mde.compare.Comparison,
	...
	protected void computeDifferences(Match match, EAttribute attribute, boolean checkOrdering) {
		...
		// Do not shortcut when many-valued FeatureMaps are affected to keep their ordering intact
	...
	protected void computeSingleValuedAttributeDifferences(Match match, EAttribute attribute) {
		...
		if (helper.matchingValues(leftValue, rightValue)) {	
			...
			if (leftValue != UNMATCHED_VALUE && comparison.isThreeWay()) {
				 ...
				 */
				if (!matchingLO && isNullOrEmptyString(originValue)) {	// 例如：_, bankc, bankc
					...
					getDiffProcessor().attributeChange(match, attribute, rightValue, DifferenceKind.CHANGE,
							DifferenceSource.RIGHT);
				} else if (!matchingLO) { // 例如：bank, bankc, bankc
		...
		} else if (match.getOrigin() != null) {	// 如果leftValue和rightValue不相等，同时origin元素不为空
			final Object originValue = safeEGet(match.getOrigin(), attribute);

			if (helper.matchingValues(leftValue, originValue)) {	// 如果leftValue和originValue相等
			...
			} else if (helper.matchingValues(rightValue, originValue)) {	// 如果rightValue和originValue相等
			...
			} else {	// 例如：Account, Account1, Account2
				/*
