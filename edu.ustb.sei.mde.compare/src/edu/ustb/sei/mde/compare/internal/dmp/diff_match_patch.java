...
package edu.ustb.sei.mde.compare.internal.dmp;
...
public class diff_match_patch {
	...
	/**
	 * How far to search for a match (0 = exact location, 1000+ = broad match). A match this many characters
	 * away from the expected location will add 1.0 to the score (0.0 is a perfect match).
	 */
	public int Match_Distance = 1000;

	/**
	 * When deleting a large block of text (over ~64 characters), how close do the contents have to be to
	 * match the expected contents. (0.0 = perfection, 1.0 = very loose). Note that Match_Threshold controls
	...
	/**
	 * Find the differences between two texts. Run a faster, slightly less optimal diff. This method allows
	 * the 'checklines' of diff_main() to be optional. Most of the time checklines is wanted, so default to
	 * true.
	 * 
	 * @param text1
	 ...
	 *            New string to be diffed.
	 * @param checklines
	 *            Speedup flag. If false, then don't run a line-level diff first to identify the changed
	 *            areas. If true, then run a faster slightly less optimal diff.
	 * @return Linked List of Diff objects.
	 */
	...
	/**
	 * Find the differences between two texts. Simplifies the problem by stripping any common prefix or suffix
	 ...
	 *            areas. If true, then run a faster slightly less optimal diff.
	 * @param deadline
	 *            Time when the diff should be complete by. Used internally for recursive calls. Users should
	 *            set DiffTimeout instead.
	 * @return Linked List of Diff objects.
	 */
	...
	/**
	 * Do a quick line-level diff on both strings, then rediff the parts for greater accuracy. This speedup
	 * can produce non-minimal diffs.
	 * 
	 * @param text1
	 *            Old string to be diffed.
	 * @param text2
	 ...
	 * diff. See Myers 1986 paper: An O(ND) Difference Algorithm and Its Variations.
	 * 
	 * @param text1
	 *            Old string to be diffed.
	 * @param text2
	...
	/**
	 * Split two texts into a list of strings. Reduce the texts to a string of hashes where each Unicode
	 ...
	 * @param text2
	 *            Second string.
	 * @return An object containing the encoded text1, the encoded text2 and the List of unique strings. The
	 *         zeroth element of the List of unique strings is intentionally blank.
	 */
	 ...
	 * @param text2
	 *            Second string.
	 * @return The number of characters common to the end of the first string and the start of the second
	 *         string.
	 */
	...
	/**
	 * Do the two texts share a substring which is at least half the length of the longer text? This speedup
	 * can produce non-minimal diffs.
	 * 
	 * @param text1
	 *            First string.
	 * @param text2
	 *            Second string.
	 * @return Five element String array, containing the prefix of text1, the suffix of text1, the prefix of
	 *         text2, the suffix of text2 and the common middle. Or null if there was no match.
	 */
	 ...
	 * @return Five element String array, containing the prefix of longtext, the suffix of longtext, the
	 *         prefix of shorttext, the suffix of shorttext and the common middle. Or null if there was no
	 *         match.
	 */
	private String[] diff_halfMatchI(String longtext, String shorttext, int i) {
		...
		if (best_common.length() * 2 >= longtext.length()) {
			return new String[] {best_longtext_a, best_longtext_b, best_shorttext_a, best_shorttext_b,
					best_common };
		} else {
			return null;
		}
	 ...
	 * edit to a word boundary. e.g: The c<ins>at c</ins>ame. -> The <ins>cat </ins>came.
	 * 
	 * @param diffs
	 *            LinkedList of Diff objects.
	 */
	...
	/**
	 * Reorder and merge like edit sections. Merge equalities. Any edit section can move as long as it doesn't
	 * cross an equality.
	 * 
	 * @param diffs
	 *            LinkedList of Diff objects.
	...
	public void diff_cleanupMerge(LinkedList<Diff> diffs) {
		 ...
		 * sideways to eliminate an equality. e.g: A<ins>BA</ins>C -> <ins>AB</ins>AC
		 */
		boolean changes = false;
		// Create a new iterator at the start.
	...
	/**
	 * loc is a location in text1, compute and return the equivalent location in text2. e.g. "The cat" vs
	 * "The big cat", 1->1, 5->8
	 * 
	 * @param diffs
	 *            LinkedList of Diff objects.
	 ...
	 * text2. E.g. =3\t-2\t+ing -> Keep 3 chars, delete 2 chars, insert 'ing'. Operations are tab-separated.
	...
	/**
	 * Locate the best instance of 'pattern' in 'text' near 'loc' using the Bitap algorithm. Returns -1 if no
	 * match found.
	 * 
	 * @param text
	 *            The text to search.
	...
	/**
	 * Compute a list of patches to turn text1 into text2. text2 is ignored, diffs are the delta between text1
	 * and text2.
	 * 
	 * @param text1
	 *            Old text
	...
	/**
	 * Merge a set of patches onto the text. Return a patched text, as well as an array of true/false values
	 * indicating which patches were applied.
	 * 
	 * @param patches
	 *            Array of Patch objects
	 * @param text
	...
	/**
	 * Add some padding on text start and end so that edges can match something. Intended to be called only
	 * from within patch_apply.
	 * 
	 * @param patches
	 *            Array of Patch objects.
	...
	/**
	 * Unescape selected chars for compatability with JavaScript's encodeURI. In speed critical applications
	 * this could be dropped since the receiving application will certainly decode these fine. Note that this
	 * function is case-sensitive. Thus "%3f" would not be unescaped. But this is ok because it is only called
	 * with the output of URLEncoder.encode which returns uppercase hex. Example: "%3F" -> "?", "%24" -> "$",
	 * etc.
	 *
	 * @param str
